...
description: "django의 모델 관계에 대해 알아보자."
---
...
```
- User : User = M : N -> 사용자가 사용자를 팔로우 할 수 있다.
  - `symmetrical=False`
  - 한 user가 다른 한 user를 팔로우 했다고 해서 둘이 서로 팔로우 관계라고 할 수 없다.
  - `followers` = 나를 팔로우한 user를 참조하기 위한 manager
  - `followings` = 내가 팔로우한 user를 참조하기 위한 manager
